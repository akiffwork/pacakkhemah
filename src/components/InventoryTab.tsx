  ...
  async function saveGear() {
    ...
    try {
      ...
      const data = {
        ...
        // NEW: Setup service data
        setup: setupAvailable ? {
          available: true,
          fee: Number(setupFee) || 0,
          description: setupDesc || "Setup service for this item",
        } : { available: false, fee: 0, description: "" },
      };
    ...
    <div className="space-y-8">
                ...
                {items.map(g => (
                  <div key={g.id} className="flex items-center gap-3 p-3 rounded-2xl bg-slate-50 border border-slate-100 relative group">
                    <div className="w-12 h-12 rounded-xl bg-white p-0.5 shadow-sm overflow-hidden flex-shrink-0">
                      <img src={g.img || "/pacak-khemah.png"} className="w-full h-full object-cover rounded-lg" alt={g.name} />
                    </div>
                      ...
                      <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                        ...
                        {g.setup?.available && (
                          <span className="text-[8px] font-bold text-purple-600 bg-purple-50 px-1.5 py-0.5 rounded">
                            <i className="fas fa-tools mr-1"></i>+RM {g.setup.fee}
                          </span>
                        )}
                      </div>
                    </div>
      ...
      {showGearModal && (
        <div className="fixed inset-0 bg-[#062c24]/95 backdrop-blur-md z-[600] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] p-6 md:p-8 max-w-lg w-full shadow-2xl max-h-[90vh] overflow-y-auto" style={{ scrollbarWidth: "none" }}>
            ...
            <div className="space-y-4">
              ...
              {/* NEW: Setup Service Section */}
              <div className="bg-purple-50 p-4 rounded-2xl border border-purple-100">
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    <i className="fas fa-campground text-purple-500"></i>
                    <label className="text-[9px] font-black uppercase text-purple-600">Setup Service for This Item</label>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input type="checkbox" checked={setupAvailable} onChange={e => setSetupAvailable(e.target.checked)} className="sr-only peer" />
                ...
                {setupAvailable && (
                  <div className="space-y-3">
                    <div>
                      <label className="text-[8px] font-bold text-purple-400 uppercase">Setup Fee (RM)</label>
                      <input type="number" value={setupFee} onChange={e => setSetupFee(e.target.value)} placeholder="e.g. 50" className="w-full bg-white border border-purple-200 p-2.5 rounded-xl text-sm font-bold outline-none" />
                    </div>
                    <div>
                      <label className="text-[8px] font-bold text-purple-400 uppercase">Description (optional)</label>
                      <input value={setupDesc} onChange={e => setSetupDesc(e.target.value)} placeholder="e.g. Full tent pitching" className="w-full bg-white border border-purple-200 p-2.5 rounded-xl text-sm outline-none" />
                    </div>
                  </div>
                )}
              </div>
              ...
              <button onClick={saveGear} disabled={uploading}
                className="w-full bg-[#062c24] text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-emerald-900 transition-all shadow-xl disabled:opacity-50">
                {uploading ? <i className="fas fa-spinner fa-spin"></i> : editingGear ? "Update Item" : "Add Item"}
              </button>
            </div>
          </div>
        </div>
      ...
      {showDiscModal && (
        <div className="fixed inset-0 bg-[#062c24]/95 backdrop-blur-md z-[600] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] p-8 max-w-md w-full shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-xl font-black text-[#062c24] uppercase">{editingDisc ? "Edit Discount" : "New Discount"}</h3>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select value={discType} onChange={e => setDiscType(e.target.value)} className={inputCls}>
                  <option value="nightly_discount">Extended Stay (Auto)</option>
                  <option value="promo_code">Promo Code (Manual)</option>
                </select>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="text-[9px] font-bold text-slate-400 uppercase">Discount (%)</label>
                  <input type="number" value={discPercent} onChange={e => setDiscPercent(e.target.value)} className={inputCls} />
                </div>
                {discType === "nightly_discount" ? (
                  <div>
                    <label className="text-[9px] font-bold text-slate-400 uppercase">Trigger Nights</label>
                    <input type="number" value={discTrigger} onChange={e => setDiscTrigger(e.target.value)} className={inputCls} />
                  </div>
                ) : (
                  <div>
                    <label className="text-[9px] font-bold text-slate-400 uppercase">Code</label>
                    <input value={discCode} onChange={e => setDiscCode(e.target.value.toUpperCase())} className={inputCls} placeholder="e.g. CAMPING20" />
                  </div>
                )}
              </div>
              {discType === "nightly_discount" && (
                <div className={`p-4 rounded-2xl border cursor-pointer transition-all ${discPublic ? "bg-emerald-50 border-emerald-200" : "bg-slate-50 border-slate-100"}`}
                  onClick={() => setDiscPublic(!discPublic)}>
                  <div className="flex justify-between items-center">
                    <div>
                      <p className="text-[10px] font-black uppercase text-[#062c24]">Show in Shop Banner</p>
                      <p className="text-[8px] text-slate-400">Display as "Save X% on Y+ nights!"</p>
                    </div>
                    <input type="checkbox" checked={discPublic} onChange={() => {}} className="w-4 h-4 accent-emerald-500" />
                  </div>
                </div>
              )}
              <div className="flex gap-3">
                {editingDisc && (
                  <button onClick={deleteDisc} className="flex-1 py-4 rounded-2xl font-black uppercase text-[10px] bg-red-50 text-red-500 hover:bg-red-100 transition-all">
                    Delete
                  </button>
                )}
                <button onClick={saveDisc} className="flex-1 bg-[#062c24] text-white py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:bg-emerald-900 shadow-xl transition-all">
                  {editingDisc ? "Update" : "Create"}
                </button>
              </div>
            </div>
          </div>
